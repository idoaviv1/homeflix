...
1. **חובה להחיל את השינוי אוטומטית בכל שלוש הגרסאות**:
   ...
   - **אפליקציית Galaxy S22 Ultra (Android APK)**: מסונכרנת באמצעות `npx cap sync android`, שומרת על תאימות אנדרואיד (Cleartext HTTP, Network Security Config, הרשאות אחסון ורשת), ונבנית/נשמרת כחבילת APK בנתיב `/media/windows/Shared-APK/`.
...
2. **אין להשאיר פלטפורמה מאחור**:
   ...
   - שתי תיקיות השיתוף חייבות להיות מעודכנות ומסודרות:
     ...
     - `/media/windows/Shared-APK/` עבור קובצי הגלקסי (`Homeflix-Galaxy-S22-Ultra.apk`).
